...
namespace OctaneEngine.ShellProgressBar
{
    public abstract class ProgressBarBase
    {
        ...
        public ConsoleColor ForegroundColor
        {
            get
            {
                var realColor = _dynamicForegroundColor ?? Options.ForegroundColor;
                return EndTime.HasValue
                    ? Options.ForegroundColorDone ?? realColor
                    : realColor;
            }
        ...
        public ChildProgressBar Spawn(int maxTicks, string message, ProgressBarOptions options = null)
        {
            ...
            if (options?.CollapseWhenFinished == false && Options.CollapseWhenFinished)
                options.CollapseWhenFinished = true;

            var pbar = new ChildProgressBar(
                maxTicks, message, DisplayProgress, WriteLine, WriteErrorLine, options ?? Options, d => Grow(d)
            );
            Children.Add(pbar);
            DisplayProgress();
            return pbar;
