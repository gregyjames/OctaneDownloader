...
namespace OctaneEngine.ShellProgressBar
{
    public class ProgressBar : ProgressBarBase, IProgressBar
    {
        ...
        {
            _writeMessageToConsole = Options.WriteQueuedMessage ?? DefaultConsoleWrite;
        ...
        {
            var pbarHeight = Options.DenseProgressBar ? 1 : 2;
        ...
        private void GrowDrawingAreaBasedOnChildren()
        {
            ...
            bool progressBarOnTop)
        {
            ...
            if (!string.IsNullOrWhiteSpace(ProgressBarOptions.ProgressMessageEncodingName))
                width = width + message.Length - System.Text.Encoding
                    .GetEncoding(ProgressBarOptions.ProgressMessageEncodingName).GetBytes(message).Length;
            ...
            TimeSpan estimatedDuration, bool disableBottomPercentage, string percentageFormat)
        {
            ...
            if (!string.IsNullOrWhiteSpace(ProgressBarOptions.ProgressMessageEncodingName))
                column1Width = column1Width + message.Length - System.Text.Encoding
                    .GetEncoding(ProgressBarOptions.ProgressMessageEncodingName).GetBytes(message).Length;
            ...
            if (disableBottomPercentage) truncatedMessage = StringExtensions.Excerpt(message, column1Width);
        ...
        {
            _stickyMessages.Enqueue(new ConsoleOutLine(message, true));
            DisplayProgress();
        }
        ...
        private void UpdateProgress()
        {
            ...
            else if (Options.ProgressBarOnBottom)
            {
                ProgressBarBottomHalf(mainPercentage, _startDate, null, Message, indentation,
                    Options.ProgressBarOnBottom, Options.ShowEstimatedDuration, EstimatedDuration,
                    Options.DisableBottomPercentage,
            ...
            DrawChildren(Children, indentation, ref cursorTop, Options.PercentageFormat);
        ...
        public void Dispose()
        {
            ...
            try
            {
                ...
                var newCursorTop = Math.Min(_originalWindowHeight,
                    _originalCursorTop + pbarHeight + openDescendantsPadding);
