...
{
	public class ProgressBar : OctaneEngine.ShellProgressBar.ProgressBarBase, OctaneEngine.ShellProgressBar.IProgressBar
	{
		private static readonly bool IsWindows = RuntimeInformation.IsOSPlatform(OSPlatform.Windows);

		private readonly ConsoleColor _originalColor;
		private readonly Func<OctaneEngine.ShellProgressBar.ConsoleOutLine, int> _writeMessageToConsole;
		...
		public ProgressBar(int maxTicks, string message, ConsoleColor color)
			: this(maxTicks, message, new OctaneEngine.ShellProgressBar.ProgressBarOptions {ForegroundColor = color})
		{
		}

		public ProgressBar(int maxTicks, string message, OctaneEngine.ShellProgressBar.ProgressBarOptions options = null)
			: base(maxTicks, message, options)
		{
			...
			if (this.Options.EnableTaskBarProgress)
				OctaneEngine.ShellProgressBar.TaskbarProgress.SetState(OctaneEngine.ShellProgressBar.TaskbarProgress.TaskbarStates.Normal);
		...
		protected override void Grow(OctaneEngine.ShellProgressBar.ProgressBarHeight direction)
		{
			switch (direction)
			{
				case OctaneEngine.ShellProgressBar.ProgressBarHeight.Increment:
					Interlocked.Increment(ref _visibleDescendants);
					break;
				case OctaneEngine.ShellProgressBar.ProgressBarHeight.Decrement:
			...
			bool progressBarOnTop)
		{
			...
			if (!string.IsNullOrWhiteSpace(OctaneEngine.ShellProgressBar.ProgressBarOptions.ProgressMessageEncodingName))
			{
				width = width + message.Length - System.Text.Encoding.GetEncoding(OctaneEngine.ShellProgressBar.ProgressBarOptions.ProgressMessageEncodingName).GetBytes(message).Length;
			}
			...
			TimeSpan estimatedDuration, bool disableBottomPercentage, string percentageFormat)
		{
			...
			if (!string.IsNullOrWhiteSpace(OctaneEngine.ShellProgressBar.ProgressBarOptions.ProgressMessageEncodingName))
			{
				column1Width = column1Width + message.Length - System.Text.Encoding.GetEncoding(OctaneEngine.ShellProgressBar.ProgressBarOptions.ProgressMessageEncodingName).GetBytes(message).Length;
			}
		...
		private readonly ConcurrentQueue<OctaneEngine.ShellProgressBar.ConsoleOutLine> _stickyMessages = new ConcurrentQueue<OctaneEngine.ShellProgressBar.ConsoleOutLine>();

		public override void WriteLine(string message)
		{
			_stickyMessages.Enqueue(new OctaneEngine.ShellProgressBar.ConsoleOutLine(message));
			DisplayProgress();
		}
		public override void WriteErrorLine(string message)
		{
			_stickyMessages.Enqueue(new OctaneEngine.ShellProgressBar.ConsoleOutLine(message, error: true));
			DisplayProgress();
		}
		...
		private void UpdateProgress()
		{
			...
			if (this.Options.EnableTaskBarProgress)
				OctaneEngine.ShellProgressBar.TaskbarProgress.SetValue(mainPercentage, 100);
		...
		private void WriteConsoleLine(OctaneEngine.ShellProgressBar.ConsoleOutLine m)
		{
		...
		private static int DefaultConsoleWrite(OctaneEngine.ShellProgressBar.ConsoleOutLine line)
		{
		...
		private static void DrawChildren(IEnumerable<OctaneEngine.ShellProgressBar.ChildProgressBar> children, Indentation[] indentation,
		...
		public void Dispose()
		{
			...
			if (this.Options.EnableTaskBarProgress)
				OctaneEngine.ShellProgressBar.TaskbarProgress.SetState(OctaneEngine.ShellProgressBar.TaskbarProgress.TaskbarStates.NoProgress);
		...
		{
			return new OctaneEngine.ShellProgressBar.Progress<T>(this, message, percentage);
		}
	}
}
