...
public static class Engine {
  ...
  /// <param name="parts">The number of parts (processes) needed to download the
  /// file.</param> <param name="bufferSize">The buffer size to use to download
  /// the file</param> <param name="showProgress">Show the progressbars?</param>
  /// <param name="outFile">The output file name of the download. Use 'null' to
  /// get file name from url.</param> <param name="doneCallback">Callback to
  /// handle download completion</param> <param name="progressCallback">The
  /// callback for progress if not wanting to use the built in progress</param>
  /// <param name="numRetries">Number of times to retry a failed
  /// download</param>
  public async static Task
  DownloadFile(string url, int parts, int bufferSize = 8096,
               bool showProgress = false, string outFile = null!,
               Action<Boolean> doneCallback = null!,
               Action<Double> progressCallback = null!, int numRetries = 10) {
    if (url == null)
    ...
    var httpPool = new ObjectPool<HttpClient?>(
        () => new HttpClient(new RetryHandler(
            new HttpClientHandler { Proxy = null, UseProxy = false,
                                    ...
                                    UseCookies = false },
            numRetries)) { MaxResponseContentBufferSize = bufferSize });
    ...
    using (var mmf = MemoryMappedFile.CreateFromFile(
      ...
      try {
        if (showProgress) {
          using (var pbar =
                     new ProgressBar(parts, "Downloading File...", options)) {
            await Parallel.ForEachAsync(
                ...
                async (piece, cancellationToken) => {
                  ...
                  // Get a http client from the pool and request for the content
                  // range
                  ...
                  try {
                    using (var request =
                      ...
                      request.Headers.Range =
                          new RangeHeaderValue(piece.Item1, piece.Item2);
                      ...
                      if (client != null) {
                        using (var message =
                          ...
                          if (message.IsSuccessStatusCode) {
                            ...
                            using (var streamToRead =
                                       await message.Content
                                           .ReadAsStreamAsync(cancellationToken)
                                           .ConfigureAwait(false)) {
                              // Create a memory mapped stream to the mmf with
                              // the piece offset and size equal to the response
                              // size
                              using (var streams = mmf.CreateViewStream(
                                         piece.Item1,
                                         message.Content.Headers.ContentLength!
                                             .Value,
                                         MemoryMappedFileAccess.Write)) {
                                using (var chil = pbar.Spawn(
                                           (int)Math.Round(
                                               (double)(message.Content.Headers
                                                            .ContentLength /
                                                        bufferSize)),
                                           "", childOptions)) {
                                  // Copy from the content stream to the mmf
                                  // stream var buffer = new byte[bufferSize];
                                  ...
                                  do {
                                    offset = 0;
                                    // Until the buffer is very nearly full or
          ...
          await Parallel.ForEachAsync(
              ...
              async (piece, cancellationToken) => {
                ...
                try {
                        ...
                        if (message.IsSuccessStatusCode) {
                          ...
                          using (var streamToRead =
                                     await message.Content
                                         .ReadAsStreamAsync(cancellationToken)
                                         .ConfigureAwait(false)) {
                            // Create a memory mapped stream to the mmf with the
                            // piece offset and size equal to the response size
                            using (var streams = mmf.CreateViewStream(
                                       piece.Item1,
                                       message.Content.Headers.ContentLength!
                                           .Value,
                  ...
                  httpPool.Return(client);
                  progressCallback(
                      (double)((tasksDone + 0.0) / (pieces.Count + 0.0)));
                }
              });
        }
      } catch (Exception ex) {
