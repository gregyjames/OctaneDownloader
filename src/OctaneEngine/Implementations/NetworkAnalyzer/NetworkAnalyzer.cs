...
public static class NetworkAnalyzer
{
    ...
    public static string PrettySize(long len)
    {
        ...
        string result = ZString.Format("{0:0.##} {1}", dLen, Sizes[order]);
    ...
    internal static async Task<int> GetNetworkSpeed((string,int) testFile, IHttpDownloader downloader)
    {
        ...
        using var response = await HttpClient.GetAsync(testFile.Item1, HttpCompletionOption.ResponseHeadersRead);
        response.EnsureSuccessStatusCode();
        using var stream = await response.Content.ReadAsStreamAsync();

        // Optimization: Use CopyToAsync with Stream.Null to efficiently discard data
        // and reduce system call overhead during network speed testing.
